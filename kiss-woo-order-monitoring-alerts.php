...
class WooCommerce_Order_Monitor {
    ...
    private function get_cached_order_count($minutes = 15) {
        ...
        try {
            ...
            if ($result === false || $result === null) {
                // Try optimized query class as fallback
                $result = WOOM_Optimized_Query::get_cached_order_count($minutes);
                
                if ($result === false || $result === null) {
                    error_log('[WooCommerce Order Monitor] All query methods failed, returning 0 to prevent false alerts');
                    return 0;
                }
            }
    ...
    private function send_throttled_alert($order_count, $threshold, $is_peak, $alert_type) {
        try {
            ...
            $body = $this->build_enhanced_alert_email_body([
                ...
                'daily_count' => $this->settings['daily_alert_count'] + 1, // +1 because we're about to send
            ...
            if (!$sent) {
                ...
                error_log(sprintf(
                    ...
                    implode(', ', $to),
                    $alert_type,
                    $this->settings['daily_alert_count'] + 1,
    ...
    public function handle_manual_check() {
        try {
            ...
            // Store original last_check to restore if needed
            $original_last_check = $this->settings['last_check'];
